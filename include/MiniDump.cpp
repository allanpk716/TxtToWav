...
CString GetExePath()
{
	WCHAR tempstr_mini22[256];
//	char pathbuf[260];
//	MultiByteToWideChar(CP_ACP,0,pathbuf,-1,tempstr_mini22,256);
	int  pathlen = ::GetModuleFileName(NULL,tempstr_mini22,260);
	
	// 替换掉单杠
	while(TRUE)
	{
		if(tempstr_mini22[pathlen--]=='\\')
			break;
	}
	tempstr_mini22[++pathlen]=   0x0;
	CString  fname = tempstr_mini22;
	return   fname;
}
...
LONG WINAPI CrashReportEx(LPEXCEPTION_POINTERS ExceptionInfo)
{
	...
	// 重启程序
	::GetModuleFileName(NULL, tempstr_mini22, MAX_PATH);
	USES_CONVERSION; 
    sprintf(szFileName ,   W2A(tempstr_mini22));
